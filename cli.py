"""Code Coverage Reporter — cli for file payloads."""
...
def parse_files(payload: str) -> List[Dict[str, Any]]:
    """Parse a JSON array of File payloads."""
    raw = json.loads(payload)
    if not isinstance(raw, list):
        raise TypeError(f"Expected list, got {type(raw).__name__}")
    return [CodeCli._coerce(item) for item in raw]


def aggregate_file_to_str(
    record: Dict[str, Any], indent: Optional[int] = None
) -> str:
    """Convenience wrapper — serialise a File to a JSON string."""
    if indent is None:
        return CodeCli.dumps(record)
