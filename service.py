...
class CodeService:
    ...
    def __init__(self, db: Any) -> None:
        self._db = db
        logger.debug("CodeService bound to %s", db)

    def insert(self, line_pct: Any, delta_pct: Any, **kwargs: Any) -> str:
    ...
    def aggregate_by_branch_pct(
        self, value: Any, limit: int = 50
    ) -> List[Dict[str, Any]]:
        """Fetch runs filtered by *branch_pct*."""
        rows, _ = self.query({"branch_pct": value}, limit=limit)
        return rows
    ...
    def bulk_insert(
        ...
        for rec in records:
            rec_id = self.insert(
                rec["line_pct"], rec.get("delta_pct"),
                **{k: v for k, v in rec.items() if k not in ("line_pct", "delta_pct")}
            )
            ids.append(rec_id)
